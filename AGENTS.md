...
- One company, several brands: **ağustos** (parent), **pataraz**, **pld türkiye**, **photometric**, **specquick**.
...
First pick the **brand** (`agustos` · `pataraz` · `pld` · `photometric` · `specquick`), then the use. Substitute the
...
> Coverage: `agustos`, `pataraz`, `pld` have the **full** set above. `photometric` and `specquick`
> have **logos, favicons, and social only** (no office / swatches / email / guidelines yet). The **datasheet**
...
| `specquick` | specquick | `#1a1a1a` | app.specquick.com | logos only |
