...
`ui/fonts/*.woff2` are subset web builds of these masters, generated by
`scripts/build_ui_fonts.py`. They are derivatives, not a second source: change a master here, then
re-run that script and `scripts/build_design_system.py`. Each family's `OFL.txt` is copied beside
them because the licence must travel with the files.
